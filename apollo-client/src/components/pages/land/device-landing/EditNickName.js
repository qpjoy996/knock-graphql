...
import { importAll, _historyHandler, errorHandler, charCodeLen } from "utils";
import { unityJSON, unityListen } from "utils/lib/unity";
import { PlatformContext } from "states/context/PlatformContext";
import { QUERY_MYSELF, SET_NICKNAME, TRACK_CLIENT } from "apollo/graphql/gql";
...
class EditNickName extends React.Component {
  ...
  refreshProfile = async () => {
    ...
    // userInfoSubscribe.startPolling(1500);
    ...
    return new Promise((resolve, reject) => {
      client.mutate({
        ...
        if (dt.data && dt.data.setUserNickname && dt.data.setUserNickname.code === 0) {
          return resolve({
            pass: true,
            msg: intl.messages['new_landing.set_nickname.success']
          })
        } else {
          return resolve({
            pass: false,
            msg: intl.messages['new_landing.set_nickname.fail']
          })
        }
      }).catch(error => {
        return resolve({
          pass: false,
          msg: intl.messages['new_landing.set_nickname.error'],
          error
        })
      })
    })
  ...
  finishSetting = async () => {
    ...
    // nickname changed
    let nicknameDT = await this.setNickname(nickname);
    if (nicknameDT.pass) {
      client.mutate({
        mutation: TRACK_CLIENT,
        variables: {
          input: {
            trackKey: "webSetNickname",
          }
        }
      }).then((dt) => {
      ...
      _historyHandler({ jump: "/home/games", history });
      unityJSON('showFloatWeb', avatar_json);
    } else if (nicknameDT.error) {
      let errMsg = errorHandler({
        error: nicknameDT.error,
  ...
  render () {
    ...
    const { originalNickname, intl } = this.props;

    return (
      <div className="landing-nickname" ref={el => this.nickNameRef = el}>
        {
          originalNickname ? (<></>) : (<BtnBack goBack={() => this.backToAvatar()} />)
        }
        ...
        >
          <div ref={el => this.drawerContent = el}>
            <div className="drawer-title"><span>{privacyTitle}</span></div>
